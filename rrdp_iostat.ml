...
module Blktap3_stats_wrapper = struct
	...
	let get_domid_devid_to_stats_blktap3 () : ((int * int) * t) list =
		...
		let sys_dev_backend_dir = "/sys/bus/xen-backend/devices" in
		let read_raw_blktap3_stats tdpid domid devid =
			...
			with _ ->
				None
		in
		let shm_dirs = Array.to_list (Sys.readdir sys_dev_backend_dir) in
...
let gen_metrics () =
	...
	let data_sources_low_mem_mode =
		(* let (++) = Int64.add in
                   Assign zero as we don't have low_mem_info in the new tapdisk stats struct *)
		let count = 0L (* List.fold_left (fun acc ((_, _), stats) ->
                        if stats.st_mem_mode  false then acc ++ 1L else acc
